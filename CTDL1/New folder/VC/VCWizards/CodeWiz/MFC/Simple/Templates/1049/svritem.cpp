...
BOOL [!output CLASS_NAME]::OnDraw(CDC* pDC, CSize& rSize)
{
	...
	// Представление элемента OLE в виде метафайла используется для отображения 
	// элемента в приложении-контейнере. Если приложение-контейнер было 
	// написано с применением библиотеки Microsoft Foundation Class Library, метафайл 
	// используется функцией-членом Draw соответствующего объекта COleClientItem.
	// Нет реализации по умолчанию. Эту функцию необходимо переопределить, чтобы выполнить отрисовку
	// элемента в указанном контексте устройства. 
	ASSERT(FALSE);			// удалить это после выполнения операций TODO
	return TRUE;
}
...
// сериализация [!output CLASS_NAME]
