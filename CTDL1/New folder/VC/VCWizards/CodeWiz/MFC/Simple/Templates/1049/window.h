...
[!else]
[!if CFILEDIALOG]
	[!output CLASS_NAME](BOOL bOpenFileDialog, // TRUE для FileOpen, FALSE для FileSaveAs
...
[!if CPRINTDIALOG]
	[!output CLASS_NAME](BOOL bPrintSetupOnly,
			// TRUE для настройки печати, FALSE для диалогового окна печати
			DWORD dwFlags = PD_ALLPAGES | PD_USEDEVMODECOPIES | PD_NOPAGENUMS
				| PD_HIDEPRINTTOFILE | PD_NOSELECTION,
			CWnd* pParentWnd = NULL);
[!else]
[!if CVSTOOLSLISTBOX]
	[!output CLASS_NAME](CMFCToolBarsToolsPropertyPage* pParent);
[!else]
	[!output CLASS_NAME]();
[!endif]
[!endif]
