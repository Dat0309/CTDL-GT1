...
function OnFinish(selProj, selObj)
{
	...
	try
	{
		if (wizard.FindSymbol("MANAGED") && !IsWinStoreProject(selProj) && !IsMCppProject(selProj))
			return VS_E_WIZARDBACKBUTTONPRESS;
		if (wizard.FindSymbol("MANAGED") && !PrepareToAddManagedClass(selProj))
			return VS_E_WIZARDBACKBUTTONPRESS;
		...
		if(wizard.FindSymbol("MANAGED"))
		{
			try
			{
				newclass.IsManaged = true;
				if (IsWinStoreProject(selProj))
					newclass.IsSealed = true;
			}
			catch (e)
			{ // above assignment will fail if project is not managed
			}
		}
		if (strBaseName != "")
		{
			...
			try 
			{
				oBase = newclass.Bases(strBaseName);
			} 
			catch(e)
			{
				// Eat ArgumentException here which is thrown 
				// if strBaseName is not found in list of bases.
				oBase = null; // Paranoia
			}
			
			if (oBase == null) 
			    oBase = newclass.AddBase(strBaseName, vsCMAddPositionEnd);
			if (oBase.Access != wizard.FindSymbol("ACCESS"))
			oBase.Access = wizard.FindSymbol("ACCESS");
		}

		var oConstructor = newclass.AddFunction(strClassName, vsCMFunctionConstructor, "", vsCMAddPositionEnd, vsCMAccessPublic, strImpl);
		var oDestructor;
		if(!wizard.FindSymbol("MANAGED") ||  wizard.FindSymbol("VIRTUAL_DTOR"))
			oDestructor = newclass.AddFunction("~"+strClassName, vsCMFunctionDestructor, "", vsCMAddPositionEnd, vsCMAccessPublic, strImpl);
		// VSWhidbey 383535 Make sure the function bodies are empty -- same as in the nonmanaged case
		if (wizard.FindSymbol("MANAGED"))
		{
			oConstructor.BodyText = "";

		        if(wizard.FindSymbol("VIRTUAL_DTOR")==true)
				oDestructor.BodyText = "";
		}
        if(wizard.FindSymbol("VIRTUAL_DTOR")==true)
		    oDestructor.IsVirtual = true;
		if(strBaseName != "")
		{
			try
			{
		...
		ShowTextPoint(oConstructor.StartPointOf(vsCMPartBody));
...
// SIG // AAIBAAIBAAIBADAhMAkGBSsOAwIaBQAEFA8tFlCk1fdu
// SIG // cWoLNXT450AKtQ0ZoIIVgjCCBMMwggOroAMCAQICEzMA
...
// SIG // BgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBRmb3jNl35s
// SIG // w9OR2LI8q528vGznbzBABgorBgEEAYI3AgEMMTIwMKAW
// SIG // gBQAZABlAGYAYQB1AGwAdAAuAGoAc6EWgBRodHRwOi8v
// SIG // bWljcm9zb2Z0LmNvbTANBgkqhkiG9w0BAQEFAASCAQBb
// SIG // rR+CcaNXBTo5Bbjeforqsm/hlecDjaSKnPsxb4dM5F1F
// SIG // oh4lKzXYQSWIRJAYZeUUuM020AhksMfedWx46rF0qy6/
// SIG // 5fYY6eZ8A/ij1Oo3Me7Ls+a7+lA4EKFjA1FQQb4MGLLR
// SIG // ZQ9O2JFN43weKONeCjG4lHRYbeJak/G/q+D/gAbZAPQw
// SIG // 6ne7y0ZbR9UA+ussX2LiASU4I0eLObf5MaShG/Y0sBkZ
// SIG // 6vSAPNvO4KpjIvmk4F0yeRQFSmGz6dQinpC0KFdPAfHy
// SIG // iCYorSzyP3v8PmubofgsP7nLUWPOs+Q0DRq04KEYpZP+
// SIG // M95mtSAKyRYU9UpG67Ylxm8e/26KPriWoYICKDCCAiQG
...
// SIG // NTA3WjAjBgkqhkiG9w0BCQQxFgQU4nYjaDIGQtGwR7fI
// SIG // qBNxgXVCQ5swDQYJKoZIhvcNAQEFBQAEggEAVOk+87AM
// SIG // WTcFWWXc2lQUdM8uNb9xU1DJF1H/7bQYdavuuTIbDs6r
// SIG // TGK14vb9XzCQ6Sg1X8Ipak9qA14goio5vCvDI3tbRyIb
// SIG // HjACiCmOPluVxGgeLQZeWNoqNL3usBv3WehuhVNvnDFM
// SIG // uIy/vyY5zf4Zy/lI0CceToULiSs6C/+KwZTht3xQG+w7
// SIG // xiIq3+/MNRUseizVdKmO3d/1ilNelD5B4TxP0ZH1z1bE
// SIG // Fczk+mjBLH+VowGO6JaGwQjI8mRYj77SJ5kUZzJmb1wW
// SIG // cTIXeIpwI+AfX1a8/XnAPTWPNzf+UdhXbe9RmLnwplFe
// SIG // 9NzrQlYd/wFul+PeuJXW/Zwpng==
