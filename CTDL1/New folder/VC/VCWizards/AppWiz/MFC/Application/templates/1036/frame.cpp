...
[!if ADV_DOCKING_NAVIGATION]
	...
	// Le volet de navigation est créé à gauche, donc l'ancrage à gauche est temporairement désactivé :
	EnableDocking(CBRS_ALIGN_TOP | CBRS_ALIGN_BOTTOM | CBRS_ALIGN_RIGHT);

	// Créer et configurer la barre de navigation "Outlook" :
	if (!CreateOutlookBar(m_wndNavigationBar, ID_VIEW_NAVIGATION, m_wndTree, m_wndCalendar, 250))
	{
		TRACE0("Impossible de créer le volet de navigation\n");
		return -1;      // échec de la création
	}
[!endif]
[!if ADV_DOCKING_CAPTION]

	// Créer une barre de légende :
	if (!CreateCaptionBar())
	{
		TRACE0("Impossible de créer la barre de légende\n");
		return -1;      // échec de la création
	}
[!endif]
[!if ADV_DOCKING_NAVIGATION]

	// La barre Outlook est créée, l'ancrage à gauche doit être autorisé.
...
void [!output MAIN_FRAME_CLASS]::OnFilePrintPreview()
{
	if (IsPrintPreview())
	{
		PostMessage(WM_COMMAND, AFX_ID_PREVIEW_CLOSE);  // force la fermeture du mode d'aperçu avant impression
	}
}
