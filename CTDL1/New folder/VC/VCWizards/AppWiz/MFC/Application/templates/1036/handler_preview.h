// PreviewHandler.h : déclaration du gestionnaire d'aperçu
...
protected:
	...
	{
		// créez votre contrôle d'aperçu ici
...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		if (pCtrl == NULL)
		{
			ATLTRACE2(atlTraceGeneral, 4, L"InitializeDocumentPreview : le pointeur vers le contrôle d'aperçu est NULL.\n");
			return E_POINTER;
		}
		...
		if (m_pDocument == NULL) 
		{
			ATLTRACE2(atlTraceGeneral, 4, L"InitializeDocumentPreview : le pointeur vers le document est NULL.\n");
			return E_POINTER;
		}
		...
		{
			ATLTRACE2(atlTraceGeneral, 4, L"InitializeDocumentPreview : échec de création du contrôle d'aperçu. Code d'erreur : %d\n", GetLastError());
			return E_FAIL;
		}
		return S_OK;
	}
